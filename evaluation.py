...
for gt_name in gt_list:
    gt_path = os.path.join(val_path, gt_name)
    img_list = os.listdir(gt_path)
    for name in img_list:
        img_path = os.path.join(gt_path, name)
        ...
        if encode_result[0] == gt_name:
            top1 += 1
        if gt_name in encode_result:
            top5 += 1
        total += 1
        print total, gt_name, top1, top5